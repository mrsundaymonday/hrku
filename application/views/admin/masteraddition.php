          ...
          <div class="content-wrapper">
            ...
            <div class="page-header">
              <h4 class="page-title">
                ...
                </span> Data Addition </h4>
              <nav aria-label="breadcrumb">
                <ul class="breadcrumb">
                  <li class="breadcrumb-item active" aria-current="page">
                    <span></span>Dashboard / Master / Addition <i class="mdi mdi-alert-circle-outline icon-sm text-primary align-middle"></i>
                  </li>
                </ul>
              </nav>
            </div>
              ...
              <div class="col-md-12 stretch-card grid-margin">
                <div class="card card-img-holder text-white">
                  <div class="card-body">
                    ...
                    <button type="button" class="btn btn-gradient-dark btn-fw" onclick="add_addition()">Add addition</button>
                      <!-- Modal -->
                                ...
                                <div class="modal-dialog modal-md">
                                  ...
                                  <div class="modal-content">
                                    <div class="modal-header">
                                      <h4 class="modal-title">Add data addition</h4>
                                    </div>
                      ...
                      <table class="table" id="example">
                        <thead>
                          ...
                          <?php
                           ...
                           if (empty($addition)) { ?>
                              <tr>
                                <th scodpe="row" colspan="9">Tidak ada daftar addition</th>
                              </tr>
                          <?php } else{
                                  foreach ($addition as $value) {?>
                                    <tr>
                                      ...
                                      <td>
                                         <a href="#" style="color: #5e7188; font-size: 40px;" id="btnedit" onclick="edit_addition(<?=$value->id_pemasukan;?>)"><i class="mdi mdi-pencil-box-outline"></i></a>
                                      </td>
                                      <td>
                                          <a href="#" style="color: #5e7188; font-size: 40px;" id="btnupdate" onclick="delete_addition(<?=$value->id_pemasukan;?>)"><i class="mdi mdi-trash-can"></i></a>
                                     </td>
                                    </tr>
                              <?php 
                                    $no++;
...
<script type="text/javascript">
    ...
    function add_addition()
    {
     // alert('test');
      ...
      $('#additional').show();
      $('.modal-title').text('Master addition'); // Set title to Bootstrap modal title
    ...
    //$('.modal-title').text('Add Person'); // Set Title to Bootstrap modal title
      ...
      $("#form").submit(function(event) {
          var url = "<?php echo site_url('main/save_addition')?>";
         swal({
              title: "Simpan data?", 
              type: "info",
                ...
                $.ajax({
                    ...
                    success: function(data) { }
                      ...
                      swal({
                          ...
                          window.location.href = "<?php echo site_url('main/addition')?>"
                      });
                  }).error(function(data) {
                      swal({
                          title: "Error", 
    ...
    function edit_addition(id){
      $('.modal-title').text('Edit Master addition'); // Set title to Bootstrap modal title
        save_method = 'update';
        $('#form')[0].reset(); // reset form on modals

        //Ajax Load data from ajax
        $.ajax({
          url : "<?php echo site_url('master2/edit_addition/')?>" + id,
          type: "GET",
          dataType: "JSON",
          success: function(data)
          {
              $('[name="id_addition"]').val(data.id_pemasukan);
              $('[name="nama_addition"]').val(data.nama_addition);
   ...
   function(isConfirm){ 
      if (isConfirm) {       
        ...
        $.ajax({
            type: "post",
            url : "<?php echo site_url('master2/delete_addition')?>/"+id,
            dataType: "JSON",
            success: function(data) { }
          }).done(function(data) {
              swal({
                  ...
                  window.location.href = "<?php echo site_url('master2/addition')?>"
              });
          }).error(function(data) {
              swal({
                  title: "Error", 
...
/*var lngtxt=($(this).find('input[name="komponen[]"]').val()).length;
        console.log(lngtxt);
        if (lngtxt==0){
